...
def main(
    ...
    final_input_path_str = resolve_param(
        str(input_csv) if input_csv else None, file_config.get("input_csv"), None
    )
    ...
    final_output_dir = resolve_param(
        output_dir,
        Path(file_config.get("output_dir")) if file_config.get("output_dir") else None,
        None,
    )

    # Output Image Resolution
    raw_output_image = resolve_param(
        str(output_image) if output_image else None,
        file_config.get("output_image"),
        "result.png",
    )
    ...
    final_output_html = resolve_param(
        output_html, Path(config_output_html) if config_output_html else None, None
    )

    final_smiles_col = resolve_param(
        smiles_col, file_config.get("smiles_col"), "smiles"
    )
    ...
    final_remove_hs = resolve_param(
        remove_hs, file_config.get("remove_hs"), None
    )  # Default is handled by mols2grid usually (True)
    ...
    final_n_items_per_page = resolve_param(
        n_items_per_page, file_config.get("n_items_per_page"), None
    )
    final_transparent = resolve_param(
        transparent, file_config.get("transparent"), False
    )
    ...
    grid_kwargs = {}
    if final_sort_by is not None:
        grid_kwargs["sort_by"] = final_sort_by
    if final_remove_hs is not None:
        grid_kwargs["removeHs"] = final_remove_hs
    if final_use_coords is not None:
        grid_kwargs["use_coords"] = final_use_coords
    if final_coord_gen is not None:
        grid_kwargs["coordGen"] = final_coord_gen
    if final_border is not None:
        grid_kwargs["border"] = final_border
    if final_gap is not None:
        grid_kwargs["gap"] = final_gap
    if final_fontfamily is not None:
        grid_kwargs["fontfamily"] = final_fontfamily
    if final_text_align is not None:
        grid_kwargs["text_align"] = final_text_align
    if final_transparent:
        grid_kwargs["transparent"] = True

    # Batch Processing Logic
    total_rows = len(df)
    chunk_size = (
        final_n_items_per_page
        if (final_n_items_per_page and final_n_items_per_page > 0)
        else total_rows
    )
    ...
    padding_width = len(
        str(num_chunks)
    )  # Calculate padding width based on total chunks
    if num_chunks == 0:
        num_chunks = 1  # Handle empty df case gracefully-ish
    ...
    for i in tqdm(range(num_chunks), desc="Processing Batches"):
        ...
        if num_chunks > 1:
            ...
            # Use zero padding
            current_output_path = (
                final_output_image.parent / f"{stem}_{i + 1:0{padding_width}d}{suffix}"
            )
        else:
